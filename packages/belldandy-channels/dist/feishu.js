...
export class FeishuChannel {
        ...
        try {
            ...
            if (message.message_type === "text") {
                ...
                if (Buffer.isBuffer(response)) {
                    ...
                    const chunks = [];
                    const stream = await response.response.blob?.()?.stream?.(); // Modern?
                    // Fallback:
        ...
        try {
            ...
            for await (const item of stream) {
                if (item.type === "delta") {
                    ...
                    console.log(`Feishu: Tool result: ${item.name} - success: ${item.success}`, item.success ? item.output?.slice(0, 100) : item.error);
                }
            }
            if (replyText) {
     ...
     * @returns 是否发送成功
     */
    async sendProactiveMessage(content, chatId) {
