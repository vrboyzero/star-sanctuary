...
export const taskGraphValidatingTool: Tool = {
  definition: {
    name: "task_graph_validating",
    description: "将某个 task graph 节点置为 validating，表示进入验证状态。",
    parameters: {
      type: "object",
      properties: {
        ...
        summary: { type: "string", description: "可选，验证摘要。" },
  ...
  async execute(args: JsonObject, context: ToolContext) {
    const name = "task_graph_validating";
    ...
    try {
      ...
      return ok(name, `节点已进入 validating。\n\n${formatTaskNode(result.node)}`);
    } catch (err) {
      return fail(name, `切换 validating 失败: ${err instanceof Error ? err.message : String(err)}`);
    }
  },
};
