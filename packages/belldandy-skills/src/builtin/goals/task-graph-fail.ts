...
export const taskGraphFailTool: Tool = {
  definition: {
    name: "task_graph_fail",
    description: "将某个 task graph 节点置为 failed，表示该节点执行失败。",
    parameters: {
      type: "object",
      properties: {
        ...
        summary: { type: "string", description: "可选，失败摘要。" },
        block_reason: { type: "string", description: "可选，失败原因。" },
  ...
  async execute(args: JsonObject, context: ToolContext) {
    const name = "task_graph_fail";
    if (!context.goalCapabilities?.failTaskNode) {
    ...
    try {
      const result = await context.goalCapabilities.failTaskNode(goalId, nodeId, {
      ...
      return ok(name, `节点已标记为 failed。\n\n${formatTaskNode(result.node)}`);
    } catch (err) {
      return fail(name, `标记 failed 失败: ${err instanceof Error ? err.message : String(err)}`);
    }
  },
};
