...
export const taskGraphPendingReviewTool: Tool = {
  definition: {
    name: "task_graph_pending_review",
    description: "将某个 task graph 节点置为 pending_review，表示进入待评审状态。",
    parameters: {
      type: "object",
      properties: {
        ...
        summary: { type: "string", description: "可选，评审摘要。" },
  ...
  async execute(args: JsonObject, context: ToolContext) {
    const name = "task_graph_pending_review";
    if (!context.goalCapabilities?.markTaskNodePendingReview) {
    ...
    try {
      const result = await context.goalCapabilities.markTaskNodePendingReview(goalId, nodeId, {
      ...
      return ok(name, `节点已进入 pending_review。\n\n${formatTaskNode(result.node)}`);
    } catch (err) {
      return fail(name, `切换 pending_review 失败: ${err instanceof Error ? err.message : String(err)}`);
    }
  },
};
