...
export const goalCheckpointExpireTool: Tool = {
  definition: {
    name: "goal_checkpoint_expire",
    description: "将当前长期任务节点的 checkpoint 标记为 expired。",
    parameters: {
      type: "object",
      properties: {
        goal_id: { type: "string", description: "可选，目标 ID；默认从当前 goal 会话推断。" },
        node_id: { type: "string", description: "节点 ID。" },
        checkpoint_id: { type: "string", description: "可选，指定 checkpoint ID；默认取该节点最近待处理项。" },
        summary: { type: "string", description: "可选，超时/过期摘要。" },
        note: { type: "string", description: "可选，过期原因或备注。" },
        ...
        decided_by: { type: "string", description: "可选，操作人 / 审批人标识。" },
  ...
  async execute(args: JsonObject, context: ToolContext) {
    const name = "goal_checkpoint_expire";
    if (!context.goalCapabilities?.expireCheckpoint) {
    ...
    try {
      const result = await context.goalCapabilities.expireCheckpoint(goalId, nodeId, {
      ...
      return ok(name, `已标记 checkpoint 过期。\n\n${formatCheckpoint(result.checkpoint)}\n\n${formatTaskNode(result.node)}`);
    } catch (err) {
      return fail(name, `标记 checkpoint 过期失败: ${err instanceof Error ? err.message : String(err)}`);
    }
  },
};
