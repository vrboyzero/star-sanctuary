...
export const taskGraphClaimTool: Tool = {
  definition: {
    name: "task_graph_claim",
    description: "将某个 task graph 节点置为 in_progress，表示开始执行。",
    parameters: {
      type: "object",
      properties: {
        ...
        summary: { type: "string", description: "可选，本次启动摘要。" },
        artifacts: { type: "array", description: "可选，本次补充的产物路径。", items: { type: "string" } },
        checkpoint_status: {
          type: "string",
  ...
  async execute(args: JsonObject, context: ToolContext) {
    ...
    try {
      ...
      return ok(name, `已认领并启动节点。\n\n${formatTaskNode(result.node)}`);
    } catch (err) {
      return fail(name, `启动 task graph 节点失败: ${err instanceof Error ? err.message : String(err)}`);
    }
  },
};
