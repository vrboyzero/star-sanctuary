...
export class MemoryStore {
    ...
    `);
        stmt.run(chunk.id, chunk.sourcePath, chunk.sourceType, chunk.memoryType, chunk.startLine ?? null, chunk.endLine ?? null, chunk.content, JSON.stringify(chunk.metadata ?? {}), chunk.channel ?? null, chunk.topic ?? null, chunk.tsDate ?? null, chunk.category ?? null, chunk.agentId ?? null, now, now);
    }
    /** 按来源路径删除 chunks */
    ...
    /** 检查 session 是否已提取过记忆 */
    isSessionMemoryExtracted(sessionKey) {
        this.ensureOpen();
        const row = this.db.prepare(`SELECT value FROM meta WHERE key = ?`).get(`memory_extracted:${sessionKey}`);
        return row?.value === "true";
    }
    /** 标记 session 已提取记忆 */
    markSessionMemoryExtracted(sessionKey) {
        this.ensureOpen();
        const stmt = this.db.prepare(`
     ...
     */
    getChunksBySource(sourcePath, maxPerSource = 10) {
        this.ensureOpen();
        const stmt = this.db.prepare(`
      ...
      LIMIT ?
    `);
        const rows = stmt.all(sourcePath, maxPerSource);
     ...
     */
    getChunksNeedingSummary(minContentLength = 500, limit = 20) {
        this.ensureOpen();
        const stmt = this.db.prepare(`
      ...
      LIMIT ?
    `);
        return stmt.all(minContentLength, limit);
    }
    /**
     * 更新 chunk 的摘要
     */
    updateChunkSummary(chunkId, summary, summaryTokens) {
        this.ensureOpen();
        const stmt = this.db.prepare(`
    ...
    getSummaryStatus() {
        this.ensureOpen();
        const total = this.db.prepare(`SELECT COUNT(*) as c FROM chunks`).get().c;
        const summarized = this.db.prepare(`SELECT COUNT(*) as c FROM chunks WHERE summary IS NOT NULL`).get().c;
