...
export class MemoryManager {
     ...
     */
    applyDeepRetrieval(firstRound, limit) {
        // 按 source_path 分组统计
    ...
    async generateSummaries() {
        ...
        while (true) {
            ...
            const chunks = this.store.getChunksNeedingSummary(this.summaryMinContentLength, this.summaryBatchSize);
            if (chunks.length === 0)
                break;
     ...
     */
    async callLLMForSummary(content) {
     ...
     * @returns 提取并写入的记忆条数
     */
    async extractMemoriesFromConversation(sessionKey, messages) {
     ...
     * 调用 LLM 从对话中提取记忆
     */
    async callLLMForExtraction(conversationText) {
...
只输出 JSON，不要其他内容。`
     ...
     * 返回本次生成的摘要数。
     */
    async runIdleSummaries() {
        if (!this.summaryEnabled || this._paused || this._summaryRunning)
            return 0;
