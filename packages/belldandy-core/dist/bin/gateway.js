...
if (agentRegistry && toolsEnabled) {
    ...
    toolExecutor.setAgentCapabilities({
        ...
        spawnParallel: (tasks) => orchestrator.spawnParallel(tasks.map((t) => ({
            parentConversationId: t.parentConversationId ?? "system",
            agentId: t.agentId,
            instruction: t.instruction,
            context: t.context,
        }))),
        listSessions: (parentConversationId) => Promise.resolve(orchestrator.listSessions(parentConversationId)),
    });
...
if (toolsEnabled) {
    ...
    hookRegistry.register({
        ...
        handler: async (event, ctx) => {
            ...
            if (!toolName || !AUTO_BOUNDARY_TOOLS.has(toolName))
                return;
            const sessionKey = ctx.sessionKey;
            if (!sessionKey)
                return;
    ...
    try {
        ...
        feishuChannel = new FeishuChannel({
            ...
            sttTranscribe: async (opts) => {
                const result = await transcribeSpeech(opts);
                if (result)
                    logger.info("feishu", `Transcribed audio (${result.durationSec?.toFixed(1) ?? "?"}s) from ${result.provider}`);
                return result;
            },
        });
