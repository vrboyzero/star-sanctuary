...
在第二阶段已经完成“可看、可追溯、可验证”之后，下一阶段最适合推进的不是继续扩 Viewer，而是把多 Agent 协作里真正高价值的一步补上：

- **显式共享记忆**

目标不是做开放式公共知识库，而是在现有 `agentId` 隔离之上，加一个**可控、可审计、默认关闭**的共享层。
...
- `Memory Viewer`
- 真实运行态可观察性
- `toolCalls / artifactPaths / memoryLinks` 追溯闭环

这意味着如果共享记忆出了问题，现在已经有能力看见、定位、回滚；因此第三阶段适合进入“最小共享层”。

### 27.2 第三阶段目标

第三阶段最小版只解决 3 件事：

1. 允许 Agent 把自己的部分记忆显式提升为 `shared`
2. 允许其他 Agent 在显式指定 `scope` 时检索到这些共享记忆
3. 保持默认行为不变，不让私有记忆被误召回

### 27.3 边界

第三阶段最小版明确**不做**：

- 不做自动 public/shared 提升
- 不做独立共享库或第二套 DB
- 不做复杂 ACL / 成员组 / 组织级权限
- 不做 `global` 的完整产品化设计
- 不做共享记忆自动写回 `Method / Skill`
- 不做大规模 Viewer 管理后台
...
- 现有 `agent_id` 继续保留，负责“归属 Agent”
- 现有 `agent_id IS NULL` 的历史/system 记忆继续按“系统级可见”处理
- 第三阶段不急着把 `global` 做成正式枚举值，避免把共享层复杂度一次拉高
...
1. **增强 `memory_search`**
   ...
   - 建议取值：
     - `private`：默认，保持当前行为
     - `shared`：只看共享层 + 系统级记忆
     - `all`：当前 Agent 私有 + 共享层 + 系统级记忆

2. **新增 `memory_share_promote`**
   - 用于把一条 chunk 或一个来源文件下的记忆显式提升为 `shared`
   - 输入建议支持：
     - `chunk_id`
     - `source_path`
   - 第一版不做 `demote` 也可以，但建议预留

如果还要再收敛一步，可以先不加新工具名，而是在 `memory_write` / `memory_search` 之外只补一个：
...
但 Viewer 过滤不是第三阶段最小必做项。

### 27.7 验收点

第三阶段最小版只看这些结果是否成立：

- 默认检索行为与现在一致，不出现私有记忆泄漏
- 某条记忆被显式 promote 后，其他 Agent 可通过 `scope=shared` 检索到
- 未 promote 的记忆不会被其他 Agent 检索到
- 历史 `agent_id IS NULL` 数据兼容，不出现“记忆失忆”
- `memoryLinks`、`Tasks`、`Viewer` 不因共享层引入明显回归
...
2. **共享必须显式触发**
   - 不做自动提升
3. **检索必须显式 scope**
   - 避免共享记忆悄悄污染当前 Agent 的默认工作上下文
4. **先做 promote，不急着做治理后台**
   - 先验证共享层是否有真实协作价值，再决定是否扩 Viewer 管理

### 27.9 当前建议

第三阶段最小落地版本建议进一步收敛成一句话：

- **先做“显式共享 + 显式共享检索”，不要一开始做成公共知识空间。**

这样做的好处是：

- 与现有 `agentId` 隔离模型冲突最小
- 行为边界清楚，最容易做灰度
- 一旦价值成立，后续再扩 `Viewer` 过滤、共享治理、经验提升都会更稳
...
1. 数据层补 `visibility`
2. `memory_search` 增加 `scope`
3. 增加 `memory_share_promote`
4. 做双 Agent 手测
5. 再决定是否补 Viewer 的 `shared` 过滤与标记

### 27.11 当前结论

从当前进度往下走，第三阶段最合理的目标不是“更复杂的记忆系统”，而是：

- **让 Belldandy 首次具备受控的多 Agent 共享记忆能力，同时保持默认私有与可追溯。**

如果第二阶段解决的是“看得见”，那么第三阶段最小版要解决的就是：
...
- 给 `chunks` 增加最小共享语义，但不改变现有私有检索行为
...
- 旧库自动迁移成功
- 历史数据默认按 `private` 或兼容逻辑处理，不出现检索回归
- 不修改 `agent_id` 现有语义
...
- 在不影响默认检索的前提下，让其他 Agent 可以显式检索共享记忆
...
- 默认不传 `scope` 时，行为与当前一致
- `scope=shared` 时，只返回共享层 + 系统级记忆
- `scope=all` 时，可返回“当前 Agent 私有 + 共享层 + 系统级记忆”
- 未共享的私有记忆不会被其他 Agent 检索到
...
- 复用 `chunks.visibility`

**接口 / 工具**

- 新增 `memory_share_promote`
  - 输入：
    - `chunk_id`
    - 或 `source_path`
...
- 能把单条 chunk 提升为 `shared`
- 能把某个来源文件下的 chunk 批量提升为 `shared`
- promote 后，默认当前 Agent 检索行为不异常
- promote 后，其他 Agent 只有在显式 `scope=shared` / `all` 时才能看到

#### P3-4 手测与灰度验证

**目标**

- 在真实多 Agent 场景下验证共享层的实际价值与边界
...
- 主要依赖现有：
  - `WebChat`
  - `Memory Viewer`
  - `task_recent / task_get`

**手测路径**

1. Agent A 写入或选定一条记忆
2. Agent A 调用 `memory_share_promote`
3. Agent B 调用 `memory_search scope=shared`
4. Agent B 调用默认 `memory_search`
5. 对比两次结果

**验收点**

- `scope=shared` 能命中共享记忆
- 默认 `memory_search` 不会误命中对方私有记忆
- `Tasks` 与 `memoryLinks` 不因共享层出现明显异常

#### P3-5 可选补充：Viewer 共享标记
...
- 不一定新增接口，可先复用已有查询结果字段

**验收点**

- `Memory Viewer` 中能看到 `private/shared` 标记
- 可按 `shared` 做轻量过滤
- 不引入新的编辑复杂度
...
3. **P3-3 写入层：显式提升共享**
4. **P3-4 手测与灰度验证**
5. **P3-5 Viewer 共享标记（可选）**

### 27.14 当前建议（排期结论）

第三阶段最小版建议收敛成一句话：

- **先把共享层做成“默认私有、显式 promote、显式 scope 检索”的三件套，再决定是否补 Viewer 标记。**

这样安排的原因是：

- 数据层与检索层最先决定系统边界
- promote 工具决定共享是否真正可用
- Viewer 标记不是第三阶段成立的前置条件，只是后续体验增强
