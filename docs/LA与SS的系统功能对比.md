...
| 程序化工具调用（PTC） | 是核心架构能力 | 已补第一版最小 runtime，但仍不是核心范式 | 高 |
| 持久工作区 | 强，且和专属沙箱强绑定 | 强，但偏本地文件工作区 | 中 |
| 代理群 | 强，且支持中途 steering / resume | 已有并行子任务，并补了第一版最小 steering / resume 闭环，但仍弱于 LangAlpha | 高 |
| 实时导航 | 强 | 已补第一版 safe-point steering 与 finished-task resume，但仍不是 mid-run steering | 高 |
...
4. 已补第一版最小可控 PTC runtime。
   - `packages/belldandy-skills/src/builtin/ptc-runtime/index.ts`
   - 当前做法是：
     - 使用受控 JS VM 执行同步脚本
     - 只允许脚本通过声明过的 `inputs` 读取工作区内文件
     - 只允许向 `generated/ptc-runs/<runId>/artifacts/` 写产物
     - 自动落盘 `script.js / manifest.json / result.json`

5. 已补第一版 PTC helper。
   - `packages/belldandy-skills/src/builtin/ptc-runtime/helpers.ts`
   - 当前做法是：
     - 提供 `ptc.helpers.mcp` 做 MCP 结果归并与摘要
     - 提供 `ptc.helpers.records` 做记录型 JSON 的统计、分组和排序
     - 提供 `ptc.helpers.report` 做 Markdown / JSON 报表输出
     - 提供 3 个更窄的模板：
...
- `code_interpreter` 仍是通用 Python/JS 运行器，不天然知道 MCP schema
- 当前 `ptc_runtime` 虽然已有首批 helper，但还没有生成 MCP / memory / conversation / task 的专用 domain wrapper
- 还没有形成 LangAlpha 那种“代码直接 import MCP wrapper”的完整工作流

### 差异判断

Star Sanctuary 现在已经不只是“有代码执行能力”，而是有了第一版最小可控 PTC runtime，但还没有把它升级成完整的 “MCP 数据编排层”。
...
- 也能“调用 MCP”
- 也开始能“受控处理本地结构化数据”
- 但还不能像 LangAlpha 一样，让代理稳定地把 MCP wrapper、批量数据拉取和程序化分析组合成完整的低 token 高数据密度范式
...
6. 已补第一版最小 steering channel：
   - gateway / query runtime 支持 `subtask.update`
   - WebChat 可在 subtasks detail 区对运行中的子任务发送 steering
   - runtime 会记录 `accepted / delivered / failed` 状态
7. 已补第一版最小 resume channel：
   - gateway / query runtime 支持 `subtask.resume`
   - 已结束子任务可在 subtasks detail 区直接 resume
   - runtime 会记录 `accepted / delivered / failed` resume 状态

### 差异判断

Star Sanctuary 已有“并行子任务编排”，并补了第一版最小 steering / resume 闭环，但比 LangAlpha 仍少两块关键能力：

1. 基于 checkpoint 的 resume / continuation
2. 子代理级上下文恢复与长生命周期管理
...
- 有状态面板
- 有工作树隔离
- 有 safe-point steering / relaunch
- 有 finished-task resume / relaunch
...
2. `高优先级`
   ...
   - 可恢复输入
   - 当前状态：
     - 第一版最小 `continuationState` 已落到 `subtask.get` 与 `goal.handoff.generate`
     - 已统一 `summary / nextAction / checkpoints / progress / recommendedTarget`
     - 仍不是完整 checkpoint replay / cross-agent takeover runtime
...
Star Sanctuary 现在已经补上第一版最小 steering channel，但仍不是 LangAlpha 这种同一运行内的 mid-run steering：
...
3. 子任务可查看、停止、归档
4. 运行中的子任务可通过 `subtask.update` 发 steering
5. 前端已区分 steering 状态，并显示 `accepted / delivered / failed`
6. 已结束子任务可通过 `subtask.resume` 发 continuation
7. 前端已区分 resume 状态，并显示 `accepted / delivered / failed`
8. `subtask.get` 与 `goal.handoff.generate` 已输出第一版最小 `continuationState`

但我没有在当前仓库里看到与 LangAlpha 等价的这类能力：

- 给“正在运行的主代理”发送 mid-run follow-up
- 同一运行内接收 steering 并继续向前
- token 级或 tool-loop 级消费 steering，而不是 safe-point 停止后重拉起

### 差异判断

Star Sanctuary 目前已经从“只能恢复 / 观察”前进到“可做最小 steering”，但整体仍偏：

- 任务切换
- 任务恢复
- 任务观察
- safe-point steering / relaunch
- finished-task resume / relaunch
...
1. `高优先级`
   ...
   - 当前状态：`已完成第一版最小落地`
   - 当前边界：仅做到 `accepted / delivered` + relaunch，未做到同一 run 内继续向前
...
3. `中优先级`
   ...
   - 当前状态：
     - `safe-point steering` 已完成第一版最小落地
     - `finished-task resume` 已完成第一版最小落地
...
1. 建立轻量 PTC runtime
   ...
   - 当前最小实现：
     ...
     - 输入文件显式声明
     - 输出目录受控
     - run 级 `manifest/result/script` 自动落盘
