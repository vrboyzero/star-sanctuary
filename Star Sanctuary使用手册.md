...
- 这里的“升级”默认指版本升级；当前只会提示新版本，不会自动自升级
- **安装器版用户**：重新运行 `install.ps1 / install.sh`
  - Windows 最新版：`irm https://raw.githubusercontent.com/vrboyzero/star-sanctuary/main/install.ps1 | iex`
  - Windows 指定版本：`& ([scriptblock]::Create((irm https://raw.githubusercontent.com/vrboyzero/star-sanctuary/main/install.ps1))) -Version v0.2.4`
  - Windows 升级并强制重跑 setup：`& ([scriptblock]::Create((irm https://raw.githubusercontent.com/vrboyzero/star-sanctuary/main/install.ps1))) -Version v0.2.4 -ForceSetup`
  - Linux/macOS 最新版：`curl -fsSL https://raw.githubusercontent.com/vrboyzero/star-sanctuary/main/install.sh | bash`
  - Linux/macOS 指定版本：`curl -fsSL https://raw.githubusercontent.com/vrboyzero/star-sanctuary/main/install.sh | bash -s -- --version v0.2.4`
  - Linux/macOS 升级并强制重跑 setup：`curl -fsSL https://raw.githubusercontent.com/vrboyzero/star-sanctuary/main/install.sh | bash -s -- --version v0.2.4 --force-setup`
  - 当前语义：升级成功后切到新版本；失败会回滚；若安装根已存在 `.env.local`，升级时默认跳过 `bdd setup`
...
#### 7.5.1 主 Agent / 子 Agent 关系图

很多人第一次看到多 Agent，会自然以为“主 Agent”和“子 Agent”是两种不同实体。当前 Star Sanctuary 里的实际机制不是这样。

更准确地说：

- **主 / 子** 是一次运行里的**角色**
- `resident / worker` 是 Agent Profile 的**类型标签**
- **同一个 AgentProfile 可以直接接用户消息，也可以被别的 Agent 委派为子 Agent**

可以把当前结构理解成下面这张图：

```text
用户 / WebChat / HTTP / 渠道消息
        |
        v
message.send(agentId=?)
        |
        v
AgentRegistry.create(agentId)
        |
        v
某个 AgentProfile 开始运行
        |
        +------------------------------+
        |                              |
        |  不委托                      |  需要拆任务
        |                              |
        v                              v
继续在当前主会话中处理            delegate_task / delegate_parallel / sessions_spawn
                                       |
                                       v
                              SubAgentOrchestrator.spawn(...)
                                       |
                                       v
                              AgentRegistry.create(targetAgentId)
                                       |
                                       v
                           某个 AgentProfile 作为子 Agent 运行
                                       |
                                       v
                          独立 sub_xxx 会话中完成任务并返回结果
                                       |
                                       v
                               主 Agent 汇总结果后继续推进
```
...
- 上半段主会话里运行的 Agent，和下半段被委派执行的 Agent，**底层都是从同一个 `AgentRegistry` 按 `agentId` 创建出来的**
- 因此，**同一个 `agentId` 既可以在某次对话里充当主 Agent，也可以在另一次委托里充当子 Agent**

例如：

- `coder` 可以直接在 WebChat 里和你对话，作为当前主 Agent
- 同时，`coder` 也可以被 `default` 或 `researcher` 通过 `delegate_task` 委派去写代码
- 反过来，`default` 自己也可以被某个主 Agent 指定成子 Agent

#### 7.5.2 `resident` 与 `worker` 到底限制了什么

当前实现里，`resident` 和 `worker` **不是**“能不能当子 Agent”的硬限制。

它们更像是：

- `resident`：进入 Resident roster，可拥有常驻主会话、常驻状态和相关观测能力
- `worker`：语义上更偏向被委派、被编排、被收束的执行型 Agent

所以要分两层理解：

**第一层：普通对话 / 子任务编排**

- `resident` 可以直接作为主 Agent
- `worker` 也可以直接作为主 Agent
- `resident` 可以被当成子 Agent
- `worker` 也可以被当成子 Agent

也就是说，在**普通消息入口**和**子任务入口**里，当前代码并没有按 `kind` 把它们隔开。

**第二层：Resident 专属能力**

只有这一层会显式区分：

- Resident roster 只纳入 `resident`
- Resident 主会话 / 常驻 session 只允许 `resident`
- 如果某个 AgentProfile 是 `worker`，它不会进入 resident 专属那套常驻治理

因此，一句话记忆即可：

```text
主 / 子 = 运行角色
resident / worker = 治理语义
```

如果你只是想知道“同一个 Agent 能不能既被直接使用，又被别的 Agent 当子 Agent 指挥”，当前答案是：

- **可以**

如果你想知道“它能不能进入 resident 常驻主会话体系”，那就要再看它是不是 `resident`。

#### 7.5.3 多 Agent 并行做不同任务 / 不同项目

除了“一个主 Agent 委派多个子 Agent”之外，当前系统也支持你**同时指挥多个 Agent 去做不同的事情**。

可以这样理解：

- `Agent A` 可以处理 `A 项目` 的任务
- `Agent B` 可以同时处理 `B 项目` 的任务
- 两边的任务可以并行推进，不要求你先等一个做完再开始另一个

典型场景例如：

- 让 `coder` 在项目 A 修一个接口问题
- 同时让 `researcher` 在项目 B 整理调研结论
- 或者让两个不同的 `coder` / `worker` 分别推进两个独立仓库里的任务

当前这项能力的落点，主要是**任务级运行目录**，而不是“给每个 Agent 永久绑定一个完全不同的项目根目录”。

也就是说，当前更准确的说法是：

- 系统支持**多个 Agent 并行处理多个不同任务**
- 也支持它们在**不同项目目录**下工作
- 但这主要依赖任务启动时的 `cwd` / 子任务 `launchSpec` / 允许根目录控制
- **不是**按 `AgentProfile` 预先永久绑定某个外部项目目录

要让不同项目目录可访问，前提通常是：

- 目标项目位于当前主工作区内
- 或者你已经通过 `BELLDANDY_EXTRA_WORKSPACE_ROOTS` 把这些项目目录加入允许访问的根目录
...
这样一来，多个 Agent 就可以在这些允许根目录下分别处理不同项目。

如果任务是在**同一个 Git 仓库**里并行修改，为了减少互相踩改动的风险，还可以使用 `worktree` 隔离模式，让子任务进入独立的 Git worktree 目录运行。

所以，从使用者视角，你可以把当前能力理解为：

```text
你不是一次只能指挥一个 Agent 做一件事。
你可以同时让多个 Agent 去做多件事，甚至分布在多个项目目录里推进。
```

但需要记住一个当前边界：

- `workspaceBinding=custom` 目前仍更偏预留语义
- 现阶段跨项目协作主要依赖“允许根目录 + 任务级 cwd / worktree”
