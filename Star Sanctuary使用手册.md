...
4. 写入 `~/.star_sanctuary/memory/YYYY-MM-DD.md`，格式：`- [类型] 内容 (来源: session-xxx)`
...
将插件文件放到 `~/.star_sanctuary/plugins/` 目录下，Gateway 启动时会自动加载。
