...
2. [2. 安装步骤](#2-安装步骤)
3. [3. 基础配置](#3-基础配置)
   - [3.1 基础配置（必选）](#31-基础配置必选)
   - [3.1.1 Channels 路由引擎（Router MVP）配置总览（短版）](#311-channels-路由引擎router-mvp配置总览短版)
   - [3.2 可视化配置 (Settings UI)](#32-可视化配置-settings-ui)
   - [3.3 视觉与视频理解 (New!)](#33-视觉与视频理解-new)
   - [3.4 语音交互配置 (STT & TTS)](#34-语音交互配置-stt--tts)
4. [3.5 Docker 部署（推荐用于生产环境）](#35-docker-部署推荐用于生产环境)
   - [前置要求](#前置要求)
   - [快速部署](#快速部署)
   - [常用命令](#常用命令)
   - [数据备份](#数据备份)
   ...
   - [故障排查](#故障排查)
   - [详细文档](#详细文档)
5. [4. 启动与首次使用](#4-启动与首次使用)
   - [4.1 极速启动 (推荐)](#41-极速启动-推荐)
   - [4.2 使用 CLI 启动（推荐）](#42-使用-cli-启动推荐)
   - [4.3 手动启动（兼容方式）](#43-手动启动兼容方式)
   - [4.4 访问界面](#44-访问界面)
   - [4.5 首次配对（Pairing）](#45-首次配对pairing)
6. [5. 人格与记忆（让它变成你的专属 AI）](#5-人格与记忆让它变成你的专属-ai)
   - [5.1 塑造人格 (SOUL)](#51-塑造人格-soul)
   - [5.2 长期记忆 (Memory)](#52-长期记忆-memory)
   - [5.3 FACET 模组切换](#53-facet-模组切换)
7. [6. 定时任务](#6-定时任务)
   - [6.1 定时提醒 (Heartbeat)](#61-定时提醒-heartbeat)
   - [6.2 定时任务 (Cron)](#62-定时任务-cron)
8. [7. 多 Agent 系统 (Multi-Agent)](#7-多-agent-系统-multi-agent)
   - [7.1 配置 Agent Profile](#71-配置-agent-profile)
   - [7.2 Agent 专属工作区](#72-agent-专属工作区)
   - [7.3 在 WebChat 中切换 Agent](#73-在-webchat-中切换-agent)
...
9. [8. 管理命令（bdd CLI）](#8-管理命令bdd-cli)
   ...
   - [8.3 健康诊断（Doctor）](#83-健康诊断doctor)
   - [8.4 配置管理（Config）](#84-配置管理config)
   - [8.5 配对管理（Pairing）](#85-配对管理pairing)
   - [8.6 浏览器 Relay](#86-浏览器-relay)
10. [9. 社区房间（多 Agent 协作）](#9-社区房间多-agent-协作)
   - [9.1 配置社区连接](#91-配置社区连接)
   - [9.2 启动社区连接](#92-启动社区连接)
   - [9.3 多 Agent 同时连接](#93-多-agent-同时连接)
   - [9.4 动态加入房间](#94-动态加入房间)
   - [9.5 离开房间](#95-离开房间)
   - [9.6 工作原理](#96-工作原理)
   - [9.7 注意事项](#97-注意事项)
   - [9.8 office.goddess.ai 转发 Gateway（/api/message）](#98-officegoddessai-转发-gatewayapimessage)
   - [9.9 Webhook API（外部系统触发）](#99-webhook-api外部系统触发)
11. [10. 飞书渠道（手机可用）](#10-飞书渠道手机可用)
   - [9.1 配置飞书](#91-配置飞书)
   - [9.2 配置 Star Sanctuary](#92-配置-belldandy)
   - [9.3 使用](#93-使用)
   - [9.4 渠道架构说明](#94-渠道架构说明)
   - [9.5 Channels 路由引擎（Router MVP）](#95-channels-路由引擎router-mvp)
12. [10. 语音交互 (Voice Interaction)](#10-语音交互-voice-interaction)
   - [10.1 语音输入 (STT)](#101-语音输入-stt)
   - [10.2 语音输出 (TTS)](#102-语音输出-tts)
13. [11. 视觉感知 (Loopback Vision)](#11-视觉感知-loopback-vision)
   - [11.1 原理简介 (Loopback Vision)](#111-原理简介-loopback-vision)
   - [11.2 如何使用](#112-如何使用)
14. [12. 高级配置](#12-高级配置)
   - [12.1 工具权限与策略](#121-工具权限与策略)
   - [12.1.1 可视化工具管理 (Tool Settings)](#1211-可视化工具管理-tool-settings)
   - [12.2 模型容灾配置 (Model Failover)](#122-模型容灾配置-model-failover)
   - [12.3 对话压缩 (Context Compaction)](#123-对话压缩-context-compaction)
   - [12.4 服务重启 (Service Restart)](#124-服务重启-service-restart)
   - [12.5 日志系统 (Logs)](#125-日志系统-logs)
15. [13. 浏览器自动化](#13-浏览器自动化)
   - [13.1 启用方式](#131-启用方式)
   - [13.2 安装浏览器扩展](#132-安装浏览器扩展)
   - [13.3 连接使用](#133-连接使用)
16. [14. 方法论系统 (Methodology System)](#14-方法论系统-methodology-system)
   - [14.1 核心理念](#141-核心理念)
   - [14.2 常用工具](#142-常用工具)
   - [14.3 给用户的建议](#143-给用户的建议)
17. [15. 画布工作区 (Canvas)](#15-画布工作区-canvas)
   - [15.1 打开画布](#151-打开画布)
   - [15.2 节点类型](#152-节点类型)
   - [15.3 基本操作](#153-基本操作)
   - [15.4 节点关联与双击跳转](#154-节点关联与双击跳转)
   - [15.5 画布上下文注入](#155-画布上下文注入)
   - [15.6 ReAct 可视化](#156-react-可视化)
   - [15.7 Agent 操作画布](#157-agent-操作画布)
18. [16. MCP 支持 (Model Context Protocol)](#16-mcp-支持-model-context-protocol)
   - [16.1 启用 MCP](#161-启用-mcp)
   - [16.2 配置 MCP 服务器](#162-配置-mcp-服务器)
   - [16.3 传输类型](#163-传输类型)
   - [16.4 常用 MCP 服务器](#164-常用-mcp-服务器)
   - [16.5 工具命名](#165-工具命名)
19. [17. 插件与钩子系统 (Plugin & Hook System)](#17-插件与钩子系统-plugin--hook-system)
   - [17.1 钩子系统](#171-钩子系统)
   - [17.2 开发插件](#172-开发插件)
   - [17.3 加载插件](#173-加载插件)
20. [18. 技能系统 (Skills)](#18-技能系统-skills)
   - [18.1 技能目录](#181-技能目录)
   - [18.2 创建自定义技能](#182-创建自定义技能)
   - [18.3 准入条件 (Eligibility)](#183-准入条件-eligibility)
   - [18.4 Agent 中使用技能](#184-agent-中使用技能)
   - [18.5 完整示例](#185-完整示例)
21. [安全性](#安全性)
22. [性能](#性能)
23. [可维护性](#可维护性)
24. [输出格式](#输出格式)
25. [19. 常见问题 (FAQ)](#19-常见问题-faq)
...
Star Sanctuary 使用 **环境变量** 进行配置。为了方便管理，推荐在项目根目录创建一个名为 `.env.local` 的文件（Git 会自动忽略它，保护你的隐私）。
...
`~/.belldandy/channels-routing.json` 最小示例：
...
如果你觉得编辑文本文件太麻烦，Star Sanctuary 提供了全新的 Web 配置面板：

1.  启动 Star Sanctuary (`start.bat` 或 `./start.sh`)。
...
Star Sanctuary 的数据存储在你的用户主目录下的 `.belldandy` 文件夹中（例如 Windows 下是 `C:\Users\YourName\.belldandy`，Linux/Mac 下是 `~/.belldandy`）。
...
Star Sanctuary 会自动读取并索引 `.belldandy/MEMORY.md` 和 `.belldandy/memory/*.md` 文件。
...
# 每次最多注入条数（建议 2~5，默认 3）
# BELLDANDY_AUTO_RECALL_LIMIT=3
# 最低相关性阈值（建议 0.25~0.45，默认 0.3）
...
启用 `BELLDANDY_MEMORY_EVOLUTION_ENABLED=true` 后，Star Sanctuary 会在每次对话结束时自动分析会话内容，提取值得长期记住的信息：
...
让 Star Sanctuary 主动提醒你！编辑 `~/.belldandy/HEARTBEAT.md`：
...
修改 `~/.belldandy/agents.json` 后，需要**重启 Star Sanctuary Gateway / 主服务**，让 Agent Profile 重新加载。
...
在 `~/.belldandy/webhooks.json` 创建配置：
...
- **幂等保护**：通过 `X-Idempotency-Key` 防止重复执行（默认 10 分钟窗口）
- **Agent 路由**：支持指定不同的 Agent 处理请求
- **Prompt 模板**：支持 `{{key}}` 变量替换，灵活生成 prompt
- **默认关闭**：需要显式配置才启用，安全可控
...
```env
# Webhook 配置文件路径（默认 ~/.belldandy/webhooks.json）
BELLDANDY_WEBHOOK_CONFIG_PATH=~/.belldandy/webhooks.json

# Webhook 幂等性时间窗口（毫秒，默认 10 分钟）
BELLDANDY_WEBHOOK_IDEMPOTENCY_WINDOW_MS=600000
```

#### 使用场景

1. **CI/CD 构建通知**：构建失败时自动触发 Agent 分析日志
2. **监控告警**：服务器异常时触发 Agent 诊断问题
3. **定时任务**：每日定时生成数据分析报告
4. **外部系统集成**：任何支持 HTTP POST 的系统都可以触发 Agent

#### 详细文档

完整的 API 说明、配置示例、错误码参考、安全建议，请查看：

📖 [Webhook API 使用指南](./docs/webhook.md)
...
Star Sanctuary 采用了标准化的 **Channel 接口** 设计，使扩展新渠道变得简单：
...
创建 `~/.belldandy/channels-routing.json`：
...
每个会话的压缩状态（滚动摘要、归档摘要等）会自动保存到 `~/.belldandy/sessions/{会话ID}.compaction.json`。这意味着即使 Star Sanctuary 重启，之前的摘要也不会丢失。
...
Star Sanctuary 的运行日志保存在 `~/.belldandy/logs/` 目录，支持：
...
- 您也可以手动在 `~/.belldandy/methods/` 目录下编写 `.md` 文件，教 Star Sanctuary 做事。
...
在 `~/.belldandy/mcp.json` 中定义要连接的 MCP 服务器。Star Sanctuary 支持两种配置格式，可任选其一。
...
Star Sanctuary 提供了完整的插件系统，允许开发者扩展 Agent 的能力。

### 17.1 钩子系统

钩子是插件干预 Agent 行为的核心机制。Star Sanctuary 支持 **13 种生命周期钩子**：
...
**Q: 如何让从外网访问？**
A: Star Sanctuary 默认监听 `0.0.0.0`，在局域网内可以直接通过 IP 访问（需要配对）。若要公网访问，建议使用 Cloudflare Tunnel 或 Frp 等内网穿透工具，并务必开启 `BELLDANDY_AUTH_MODE=token` 增加安全性。
...
A: 可以。Star Sanctuary 已对 Windows 原生命令 (`copy`, `move`, `del`, `ipconfig` 等) 进行了特别支持。注意：为了安全起见，`del` 命令禁止使用 `/s` (递归) 或 `/q` (静默) 参数。

**Q: 出现 `模型调用失败（HTTP 400）：Invalid request: total message size ... exceeds limit ...` 怎么办？**
A: 这是上游接口的请求体大小限制触发，不是 `models.json` 缺少模态字段导致。当前 WebChat 已内置发送前体积拦截和图片自动压缩。若仍触发，请减少附件、降低图片分辨率、拆分消息，或先执行 `/compact` 再发送。
