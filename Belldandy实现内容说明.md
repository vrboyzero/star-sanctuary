...
- [Belldandy 实现内容说明](#belldandy-实现内容说明)
  ...
  - [📚 后续章节目录](#-后续章节目录)
  - [✅ 已完成功能模块](#-已完成功能模块)
    - [1. 基础架构与 WebChat (Phase 1-2)](#1-基础架构与-webchat-phase-1-2)
    - [2. 安全准入与 Pairing 机制 (Phase 3 \& 3.1)](#2-安全准入与-pairing-机制-phase-3--31)
    - [3. Skills 工具系统 (Phase 4)](#3-skills-工具系统-phase-4)
    - [4. Memory 记忆系统 (Phase 4 \& 4.5 \& M-Next)](#4-memory-记忆系统-phase-4--45--m-next)
    - [5. 对话上下文与防注入 (Phase 2.2)](#5-对话上下文与防注入-phase-22)
    - [6. SOUL 人格系统 (Phase 5)](#6-soul-人格系统-phase-5)
      - [Workspace 文件读取时机机制](#workspace-文件读取时机机制)
      - [文件创建规则](#文件创建规则)
      - [BOOTSTRAP.md 生命周期](#bootstrapmd-生命周期)
    - [7. 插件系统 (Plugin System) Phase 8](#7-插件系统-plugin-system-phase-8)
    - [7.1 钩子系统扩展 (Hook System Extension) Phase 8.3](#71-钩子系统扩展-hook-system-extension-phase-83)
    - [7.2 技能系统 (Skills System) P2-3](#72-技能系统-skills-system-p2-3)
    - [8. 浏览器扩展 (Phase 9)](#8-浏览器扩展-phase-9)
    - [9. 系统级操作 (System Execution) Phase 10](#9-系统级操作-system-execution-phase-10)
    - [10. 实时文件感知 (Real-time File Perception) Phase 11](#10-实时文件感知-real-time-file-perception-phase-11)
    - [11. Model Failover \& High Availability (Phase 8) \[已完成\]](#11-model-failover--high-availability-phase-8-已完成)
      - [11.5 Kimi K2.5 视觉/视频能力集成 (Vision \& Video)](#115-kimi-k25-视觉视频能力集成-vision--video)
    - [12. 性能与向量加速 (Vector Optimization) Phase 12](#12-性能与向量加速-vector-optimization-phase-12)
    - [13. 多媒体与语音 (Multimedia \& TTS) Phase 13](#13-多媒体与语音-multimedia--tts-phase-13)
    - [14. 视觉感知 (Vision) Phase 13.5](#14-视觉感知-vision-phase-135)
    - [14.5 Kimi 原生视觉与视频 (Native Multimodal Vision \& Video) Phase 13.6](#145-kimi-原生视觉与视频-native-multimodal-vision--video-phase-136)
    - [15. 方法论系统 (Methodology) Phase 14](#15-方法论系统-methodology-phase-14)
    - [16. 会话持久化与向量记忆 (Persistence \& Vector Memory) Phase 4.6](#16-会话持久化与向量记忆-persistence--vector-memory-phase-46)
    - [17. 记忆检索增强 (Memory Retrieval Enhancement) Phase 4.7](#17-记忆检索增强-memory-retrieval-enhancement-phase-47)
    - [18. 上下文自动压缩 (Context Compaction) Phase 2.3](#18-上下文自动压缩-context-compaction-phase-23)
    - [19. Canvas 可视化工作区 (Phase P2-4 / P3-3)](#19-canvas-可视化工作区-phase-p2-4--p3-3)
    - [20. 可视化配置 \& System Doctor (Phase 2.5)](#20-可视化配置--system-doctor-phase-25)
      - [功能亮点](#功能亮点)
      - [技术实现](#技术实现)
    - [20.5 可视化工具管理 (Phase 2.6: Tool \& MCP Management UI)](#205-可视化工具管理-phase-26-tool--mcp-management-ui)
    - [21. 极致美学重构 (Phase 3: Ethereal Digital UI)](#21-极致美学重构-phase-3-ethereal-digital-ui)
    - [22. 飞书渠道接入 (Phase 3.1)](#22-飞书渠道接入-phase-31)
    - [23. 渠道架构升级 (Channel Architecture) Phase 15](#23-渠道架构升级-channel-architecture-phase-15)
    - [24. Heartbeat 定时任务 (Phase 7) ✅ 已完成](#24-heartbeat-定时任务-phase-7--已完成)
      - [💡 Moltbot 对标分析：Heartbeat 模块](#-moltbot-对标分析heartbeat-模块)
    - [24.5 Cron 定时任务系统 (Phase 7.5) ✅ 方案 A 已完成](#245-cron-定时任务系统-phase-75--方案-a-已完成)
      - [🚀 方案 B 升级路径（未来按需）](#-方案-b-升级路径未来按需)
    - [25. 服务重启工具 (service\_restart) ✅ 已完成](#25-服务重启工具-service_restart--已完成)
    - [26. FACET 模组切换工具 (switch\_facet) ✅ 已完成](#26-facet-模组切换工具-switch_facet--已完成)
    - [27. 多 Agent 配置体系 (AgentProfile \& AgentRegistry) ✅ 已完成](#27-多-agent-配置体系-agentprofile--agentregistry--已完成)
    - [27.5 多 Agent 路由与前端集成 (P2-1a/b) ✅ 已完成](#275-多-agent-路由与前端集成-p2-1ab--已完成)
    - [27.6 Per-Agent Workspace 与 Facet 共存 (阶段 4) ✅ 已完成](#276-per-agent-workspace-与-facet-共存-阶段-4--已完成)
    - [28. 子 Agent 编排 (Sub-Agent Orchestration) Phase 16 ✅ MVP 已完成](#28-子-agent-编排-sub-agent-orchestration-phase-16--mvp-已完成)
    - [28.5 Agents 后续迭代 Phase 25 Step 1-3 ✅ 已完成](#285-agents-后续迭代-phase-25-step-1-3--已完成)
    - [29. MCP (Model Context Protocol) 支持 (Phase 17) ✅ 已完成](#29-mcp-model-context-protocol-支持-phase-17--已完成)
    - [30. 日志系统 (Logging System) Phase 18 \[核心已完成\]](#30-日志系统-logging-system-phase-18-核心已完成)
    - [31. CLI 框架 (CLI Framework) Phase P1-2 \[Phase A-D 已完成\]](#31-cli-框架-cli-framework-phase-p1-2-phase-a-d-已完成)
    - [32. 计时工具 (Timer Tool) ✅ 已完成](#32-计时工具-timer-tool--已完成)
    - [33. 任务级 Token 计数器 (Token Counter) ✅ 已完成](#33-任务级-token-计数器-token-counter--已完成)
  - [🚧 待实现功能规划](#-待实现功能规划)
    - [1. Local Embedding (优先级：低)](#1-local-embedding-优先级低)
    - [2. SOUL\_EVIL 彩蛋 (优先级：低)](#2-soul_evil-彩蛋-优先级低)
    - [3. Memory Flush 机制 (优先级：低)](#3-memory-flush-机制-优先级低)
    - [4. 记忆系统优化 (优先级：中) — Phase M-Next](#4-记忆系统优化-优先级中--phase-m-next)
      - [已完成部分](#已完成部分)
      - [未来规划](#未来规划)
    - [5. OS 计算机操作能力 (Computer Use Strategy) (优先级：中)](#5-os-计算机操作能力-computer-use-strategy-优先级中)
  - [🔐 安全与网络加固路线图 ✅ 已完成 (2026-02-08)](#-安全与网络加固路线图--已完成-2026-02-08)
    - [1. P0：配对绕过风险 ✅](#1-p0配对绕过风险-)
    - [2. P1：CSWSH 防护 + 安全启动 ✅](#2-p1cswsh-防护--安全启动-)
    - [3. P1：配置泄露防护 ✅](#3-p1配置泄露防护-)
    - [4. P2：危险工具 Opt-in ✅](#4-p2危险工具-opt-in-)
    - [5. P2：SSRF 防护增强 ✅](#5-p2ssrf-防护增强-)
    - [6. P3：浏览器域名控制 ⏳ 待验证](#6-p3浏览器域名控制--待验证)
  - [📚 附录：Moltbot 能力清单 (参考基准)](#-附录moltbot-能力清单-参考基准)
    - [1. 核心文件操作 (Core Coding)](#1-核心文件操作-core-coding)
    - [2. 浏览器自动化 (Browser Control)](#2-浏览器自动化-browser-control)
    - [3. 系统与执行 (System)](#3-系统与执行-system)
    - [4. 网络与数据 (Web \& Data)](#4-网络与数据-web--data)
    - [5. 多媒体生成 (Media)](#5-多媒体生成-media)
  ...
  - [📊 Moltbot vs Belldandy Agent 能力详细对比](#-moltbot-vs-belldandy-agent-能力详细对比)
    ...
    - [Moltbot Agent 工具完整清单](#moltbot-agent-工具完整清单)
      ...
      - [2. 系统命令 (Execution)](#2-系统命令-execution)
      - [3. 浏览器自动化 (Browser Control)](#3-浏览器自动化-browser-control)
      - [4. 网络与数据 (Web)](#4-网络与数据-web)
      - [5. 多媒体 (Media)](#5-多媒体-media)
  ...
  - [🐳 Docker 部署支持 (Phase N - MVP)](#-docker-部署支持-phase-n---mvp)
    ...
    - [后续规划](#后续规划)
    - [价值](#价值)
  - [18. UUID 身份验证系统 (Phase UUID)](#18-uuid-身份验证系统-phase-uuid)
    - [目标](#目标)
    - [实现内容](#实现内容-1)
      - [18.1 协议层扩展](#181-协议层扩展)
      - [18.2 服务端处理](#182-服务端处理)
      - [18.3 Agent层支持](#183-agent层支持)
      - [18.4 工具系统](#184-工具系统)
      - [18.5 前端支持](#185-前端支持)
    - [价值](#价值-1)
  - [19. 身份上下文系统 (Phase Identity Context)](#19-身份上下文系统-phase-identity-context)
    - [目标](#目标-1)
    - [核心功能](#核心功能)
      - [19.1 协议层扩展](#191-协议层扩展)
      - [19.2 新增工具](#192-新增工具)
      - [19.3 System Prompt 动态注入](#193-system-prompt-动态注入)
      - [19.4 缓存机制](#194-缓存机制)
    - [Token 优化效果](#token-优化效果)
    - [环境变量配置](#环境变量配置)
    - [使用场景](#使用场景)
    ...
    - [安全特性](#安全特性)
    - [价值](#价值-2)
  - [20. 社区渠道接入 (Community Channel) ✅ 已完成](#20-社区渠道接入-community-channel--已完成)
    - [目标](#目标-2)
    - [实现内容](#实现内容-2)
      - [20.1 CommunityChannel 渠道实现](#201-communitychannel-渠道实现)
      - [20.2 配置管理](#202-配置管理)
      - [20.3 工具集成](#203-工具集成)
      - [20.4 Gateway 初始化流程](#204-gateway-初始化流程)
      - [20.5 认证流程](#205-认证流程)
    - [关键修复记录](#关键修复记录)
    - [价值](#价值-3)
  - [21. 全局代码分析修复收敛 (2026-02-27) ✅ 已完成](#21-全局代码分析修复收敛-2026-02-27--已完成)
    - [目标](#目标-3)
    - [实现内容](#实现内容-3)
      - [21.1 P0 安全问题收敛](#211-p0-安全问题收敛)
      - [21.2 P1 风险问题收敛](#212-p1-风险问题收敛)
      - [21.3 O 类优化收敛](#213-o-类优化收敛)
    - [测试与构建结果](#测试与构建结果)
    - [价值](#价值-4)

## 📚 后续章节目录

> 以下目录对应「已完成功能模块」之后的主章节（不含子章节）。
...
8. [20. 社区渠道接入 (Community Channel) ✅ 已完成](#20-社区渠道接入-community-channel--已完成)
9. [21. 全局代码分析修复收敛 (2026-02-27) ✅ 已完成](#21-全局代码分析修复收敛-2026-02-27--已完成)
...
### 32. 计时工具 (Timer Tool) ✅ 已完成

- **目标**：为 Agent 提供精确的时间测量能力，用于性能分析、任务计时等场景。
- **状态**：✅ 已完成（2026-02-24）
- **实现内容**：
    - **timer 工具**（`packages/belldandy-skills/src/builtin/timer.ts`）：
        - 统一入口，通过 `action` 参数区分操作类型。
        - 支持 5 种操作：
            | 操作 | 说明 |
            |------|------|
            | `start` | 启动命名计时器，返回计时器 ID |
            | `stop` | 停止计时器并返回总用时（秒，保留 2 位小数） |
            | `lap` | 记录中间计时点（不停止计时器） |
            | `reset` | 重置并删除计时器 |
            | `list` | 列出所有活跃计时器及其状态 |
        - **命名计时器**：支持多个并发计时器，通过 `name` 参数区分。
        - **高精度**：使用 `performance.now()` 实现亚毫秒级精度（最小单位 0.01 秒）。
        - **状态管理**：进程内存存储（`Map<string, TimerState>`），重启后清空。
        - **中间计时**：支持多次 `lap` 操作，`stop` 时显示所有中间计时点。
    - **Gateway 集成**：
        - 在 `packages/belldandy-core/src/bin/gateway.ts` 中注册到 core 工具组（始终加载）。
        - 导出路径：`packages/belldandy-skills/src/index.ts` → `timerTool`。
- **使用示例**：
    ```typescript
    // 启动计时器
    timer({ action: "start", name: "task1" })
    // → "计时器 "task1" 已启动"

    // 记录中间计时
    timer({ action: "lap", name: "task1" })
    // → "计时器 "task1" 中间计时 #1: 3.45s"

    // 停止计时器
    timer({ action: "stop", name: "task1" })
    // → "计时器 "task1" 已停止\n总用时: 5.67s\n中间计时: 3.45s"

    // 列出所有计时器
    timer({ action: "list" })
    // → "当前计时器列表：\n- task1: 5.67s [已停止] (1 个中间计时)"
    ```
- **技术特性**：
    - **并发安全**：支持多个计时器同时运行，互不干扰。
    - **状态检查**：防止重复启动、停止未运行的计时器等错误操作。
    - **格式化输出**：时间统一格式化为秒（保留 2 位小数），便于阅读。
    - **轻量实现**：纯内存操作，无 I/O 开销，不影响现有功能。
- **价值**：
    - Agent 可以自主测量任务执行时间（如"这个构建花了多久？"）。
    - 支持性能分析场景（如"哪个步骤最耗时？"）。
