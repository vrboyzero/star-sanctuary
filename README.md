...
|---|---|---|---|
| 通过终端安装 / 升级 | `install.ps1` / `install.sh` | Release 包 | 命令行用户优先安装器版，后续升级路径也最直接。 |
| 日常聊天、使用 Agent | WebChat | `/api/message` | 普通使用直接走 WebChat。 |
| 修改模型 / API Key / 能力开关 | WebChat `⚙️ 设置` | `bdd config set` | 普通用户优先 Settings；脚本化再用 CLI。 |
| 修改高级 JSON 配置 | `~/.star_sanctuary/*.json` | `bdd configure ...` | 多 Agent、MCP、Webhook、渠道、安全策略主要都在状态目录里。 |
...
下面这些文件是当前最常见的“高级配置入口”，默认都位于 `~/.star_sanctuary`：
...
该向导会维护 `~/.star_sanctuary/community.json`，并支持为不同 Agent 配置 API Key、房间名与房间密码。
